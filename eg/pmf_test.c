...
void pack_prep(apop_data *d, apop_model *m){
    ...
    if (!Apop_settings_get_group(m, apop_mle))
        Apop_model_add_group(m, apop_mle, .parent=m, .tolerance=1e-4, .step_size=3);
}

double pack_constraint(apop_data *d, apop_model *m){return
    apop_linear_constraint(apop_data_pack(m->parameters, .all_pages='y'));}

apop_model pack_counter ={"Optimum is that each element equals its pack order", .p = pack_p, .prep=pack_prep, 
