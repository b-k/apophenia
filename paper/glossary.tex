...
[See also the list of notation  on page \pageref{notation}.]

\comment{
\paragraph{Acronyms}\vfill 2pt

\glossy{CDF}{Cumulative density function}

\glossy{MLE}{Maximum likelihood estimation}

\glossy{PDF}{Probability density function}


\paragraph{Terms}\vfill 2pt
}
...
\gloss{assertion}{A statement in a program used to make sure no errors
have occured. The statement does nothing, but should
always evaluate to being true; if it does not, the program halts with a
message about the failed assertion.}
...
\gloss{cumulative density function}{Cumulative density function. The integral of a \qv{PDF}.
...
\gloss{consistency}{A test is consistent if  the \qv{power} $\to 1$ as $n\to \infty$.}

\gloss{contrast}{A hypothesis about a linear combination of
coefficients, like $3\beta_1 - 2\beta_2 = 0$.}
...
elements of the inverse of the negation of the \qv{information matrix}; that is,
$(-\Iv)^{-1}$
...
\gloss{independent draws}{Two events $x_1$ and $x_2$(such as draws from a data set) are
independent if $P(x_1\cap x_2)$ (that is, the probability of $x_1$ and
$x_2$) is equal to $P(x_1)\cdot P(x_2)$.}

\gloss{information matrix}{The matrix of expected values of the second
...
\vocab{likelihood function}{Typically notated $L(\betav|\xv)$, the
probability that we'd have the parameters we have given some observed
data.}
...
\gloss{probability density function}{Probability density function, such as the familiar bell-curve
...
in that range. The PDF is always nonnegative. Compare with
\qv{cumulative density function}.}
