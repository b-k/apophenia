...
expressed as a matrix $\bf T$, so that $\xv$ transformed is $\xv'{\bf
T}$ or ${\bf T}\xv$. But any such projection maps $\bf 0$ to $\bf 0$. An
affine projection adds a constant, transforming $\xv$ to $\xv'{\bf T}+
K$ or ${\bf T}\xv' + K$, so $\bf 0$ now transforms to a nonzero value.}

\gloss{ANOVA}{``The analysis of variance is a statistical technique for
analyzing measurements depending on several kinds of effects operating
simultaneously, to decide which kinds of effects are important and to
estimate the effects.'' \citep{scheffe:anova}}
...
\gloss{CDF}{Cumulative density function. The integral of a \qv{PDF}.
...
(x-\xv)^n$. In the continuous case, if $x$ has distribution $p(x)$, then
...
\gloss{correlation coefficient}{Given the covariance $\sigma_{\xv\yv}$,
and variances $\sigma_\xv$ and $\sigma_\yv$,
...
\gloss{degrees of freedom}{The number of dimensions along which a set of
variables varies. If all $n$ variables are independent, then $df=n$, but if
...
such as an \qv{object file}, depends on another, such as a \qv{source
file}. When the depended-on file changes, the dependent file will need
to be re-produced.}

\gloss{dummy variable}{.}

\gloss{efficiency}{A parameter estimate that comes as close as possible
...
\gloss{generalized least squares}{The OLS model assumes that the
...
determine the optimum value for the free dimension. Then, that dimension
is held fixed and the next dimension is allowed to vary, and its optimum
found. By repeatedly iterating through the dimensions, an overall
...
of OLS using the instrumental variable will produced unbiased parameter
estimates.}
...
and the sqlite3 command-line program can be used as interpreters. Compare
with \qv{compiler}. }
...
\gloss{method}{A structure can contain pointers to functions as well as
plain data. Object-oriented programmers call a function inside a structure a method of the structure.}
...
\gloss{PDF}{Probability density function, such as the familiar bell-curve
...
\gloss{pointer}{The location of a piece of data.}
...
\qv{table}, drop an index, et cetera.}
...
By the mid-1980s, a multitude of variants on the \qv{UNIX} operating system ap-
peared; the Institute of Electrical and Electronics Engineers convened
a panel to write this standard so that programs written on one ﬂavor
of UNIX could be more easily ported to another ﬂavor. Santa Cruz
...
generator, a function that produces a sequence of numbers that seem
to have no pattern.}
...
score is the vector its derivatives: $S={\partial \ln P\over \partial \theta}$. }
...
program. If reading from the memory, this is a security hole; if writing to
the memory, this could destroy data or create system instability.
Therefore, the system catches segfaults and halts the program
immediately when the occur.}
...
immediately put at the shell's input prompt.}
...
\gloss{unbiased}{A test is unbiased if $(1-\beta)\geq \alpha\ \forall$ values of the parameter.
...
has evolved into code owned by Santa Cruz Operation. Others are cor-
rectly called \qv{POSIX}-compliant. The name does not stand for anything,
