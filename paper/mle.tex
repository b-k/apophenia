...
them via its \cind{apop\_maximum\_likelihood} function. You give it a
function and some parameters, and it will find the optimum.

\section{The \ind{log likelihood function}}	\label{the score}
...
they are \vocab{independent draws}.  The independence assumption 
...
that we have given some parameters; a \vocab{likelihood function}
...
Define the \vocab{log likelihood function} as $L=\ln P$, the \vocab{score} as
...
and the \vocab{information matrix} as
...
\paragraph{A \ind{Bernoulli} example} Say that we have nine draws from a
...
Since the information matrix is customarily the negation of the score,
...
more robust and manageable.\comment{Zero probability events are still
...
\marginalia{15}{\ind{Exponential families}}{%
	Equation \ref{exptrick} used the claim that
 the derivative of the expectation is
	${d\over d\beta}\int P dx$ and $\int {d P\over
	d\beta} dx$ are identical.
...
Now consider $E(S)$, and $E(I)$. The expected value of the Score is 
\begin{eqnarray}
...
but is valid for any \airq{exponential family}. The definition of this
term goes a bit beyond the scope of this text, but rest assured that the
\ind{Normal}, \ind{Gamma}, \ind{Beta}, \ind{Binomial}, \ind{Poisson},
...
Even if you have forgotten the calculus behind the bulk of these
equations, have a look at Equation \ref{moneyone}, which is known as the
\ind{information equality}.  Along with the above derivation that
...
\paragraph{MLEs achieve the Cram\'er-Rao lower bound} 
        \index{Cram\'er-Rao Inequality|(}		\label{cr} 
...
\comment{
A sketch of the proof: Take my word for it that $E(\hat\beta
\overline S)={1\over n}$. Since $\overline S=0$, that's equivalent to
$\cov(\hat\beta, \overline S)={1\over n}$.  Plug that into Cauchy-Schwarz:
$\var(\hat\beta)\var(\overline S)\geq {1\over n^2}$. Plug
...
Just remember: the Cram\'er-Rao lower bound is the inverse of $n$ times
...
\subsection{The Neyman-Pearson lemma} the \ind{Neyman-Pearson lemma}
...
?`remember the definition of a \ind{conditional probability}? It's based on the
...
by the Cram\'er-Rao lower bound, Equation \ref{CRLB}.
...
\paragraph{500: Simulated annealing}\index{simulated annealing} A controlled random walk.
