\startonecol \chapter{Maximum likelihood estimation} \label{mle}
\endonecol
...
\over \partial \theta}$: the derivative of the expectation is
	...
	the \ind{Normal}, \ind{Gamma}, \ind{Beta}, \ind{Binomial}, \ind{Poisson}, \&c. are all in this
	family.} 
...
guilty}. Most citizens fell that to convict a not-guilty person is a worse
...
it.\footnote{One could write $P(A|B)$ as a symmetric two-variable function:
...
function, \cind{apop\_maximum\_likelihood}---but what a function it is. It
...
\begin{itemize}
\item {\bf 000: The \ind{Nelder-Mead simplex algorithm}} Draws a polygon and then attempts to shift the corners of the polygon.
\item {\bf 100: \ind{Fletcher-Reeves Conjugate gradient algorithm}} This is the
...
\item {\bf 200: \ind{Broyden-Fletcher-Goldfarb-Shanno Conjugate gradient algorithm}}  Another conjugate gradient method, that calculates the direction of travel differently. 
\item {\bf 300: \ind{Polak-Ribiere Conjugate gradient algorithm}} Yet another conjugate gradient method. See \cite{avriel:nonlinear} for definitions of all of these methods.
\item {\bf 500: \ind{Simulated annealing}} A controlled random walk.
...
along, not actual values of the function.\index{plotting}
...
ratio test}
As intimated by the \ind{Neyman-Pearson lemma}, the ratio of two
...
\end{lstlisting}
Since the \cinline{apop\_model} is defined when you \cinline{\#include
$<$apophenia/headers.h$>$}, the compiler already knows all the necessary
...
\section{\treesymbol{} Non-stats modeling}    \label{econ101}
