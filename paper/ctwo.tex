...
\paragraph{Switch}\ckeyind{switch} The \ci{switch} statement is a clean way to
...
\cind{apop\_matrix\_apply} function to apply a function to every row of
a data set. If \cind{apop\_opts.thread\_count}\ci{>1}, then the
...
\section{Data structures} Let us say that you have a few million
...
loses one \vocab{util} from energy expended.\footnote{Economists
frequently refer to the utility that an agent gains from an action. 
The util is a joke unit of measurement for agents' utility levels.}
...
Listing \ref{birds.c} shows half of a program to simulate the game
among a flock of birds.  Begin reading at the \ci{main} function at the
end. It begins by calling \ci{setup}, and then, for each period until
...
Some of these functions are required by the files handling data structures
below, so the function headers should all be in a header file, for
easy reference:

\birdfig{birds.h}{Header file for the birds.}

You can see that the header file expects the existence of a few more
functions, that represent the basics for any data structure: functions
for initialization, adding elements, deleting elements, and finding
elements. Below, you will also see functions for iterating through the
entire flock.

As you will see, each data structure handles these operations differently.
...
The real failing of arrays comes in adding and deleting elements. For
adding elements, the simulation has to call \ci{realloc} every time the
...
An array can not have a hole in the middle, so elements can not just be
...
As for finding an element, we got lucky here because the \ci{id} could
be matched one-to-one with the array indices. Notice how
...
\subsection{Linked lists} A linked list is a set \ci{struct}s connected
...
simple: create a new node anywhere, and replace the \ci{NULL} pointer at
the end with a pointer to the new node. Deleting a node is also simple:
take the pointer pointing to the node, and pass it through so that it
instead points to the node's successor.
...
thousandth member of the \ci{flock} is via 9,999 \ci{->next}
steps from the head of the list. In fact, if you compile and run this
program, you will see that it runs much more slowly than the tree and
array versions.
...
The process of adding an element is also complicated by the fact that
a tree is only useful if it remains \airq{balanced}, meaning that
there are about as many left-pointing elements as right-pointing
elements. After all, if every element were to the left of the element
above it, you would just have a linked list.
...
\ci{realloc}ing, although maintaining balance may induce some shuffling. 
