...
fixed array, since each birth may require a \ci{realloc}, as would each
...
But adding an element to a linked list is simple: create a new node
anywhere, and replace the \ci{NULL} pointer at the end with a pointer to
...
easy: \ci{agent\_array[10000]}. But the only way to refer to the ten
thousandth member of the \ci{agent\_list} is via 9,999 \ci{->next}
...
\airq{balanced}, meaning that there are about as many left-pointing
...
If your agents have a natural order such that a key makes sense, and
you expect to retrieve them by key, then you will want to store them in
a tree.

code and notes on \ind{glib} here.
