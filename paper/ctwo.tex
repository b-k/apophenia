...
\subsection{Linked lists} A linked list is a set of \ci{struct}s connected
...
other details.  The \bi{make} program takes care of these things for
...
\bi{make} assembles the elaborate command line required to invoke
\ci{gcc}. Because assembling that command line by hand is so time
consuming and error-prone, you are strongly encouraged to have a Makefile
for each program.  Many programs, such as \bind{gdb}, \ind{vi} and
\ind{EMACS}, will even let you run \bi{make} without leaving the program.

For concreteness, 
Figure \ref{makefig} is a Makefile for a program named \ci{run\_me},
...
primarily grouped at the top of the file. When \bi{make} sees text of
the form \ci{\$(VAR)} in the Makefile, then it will substitute the
definition of \ci{VAR} into that point. This is very much in the
blind text-replacement style of the C preprocessor. The \ci{\$<} and
\ci{\$@} variables are special variables that indicate the dependency
...
actions. A \airq{target} is something to be produced, such as an object
file or an executable. On page \pageref{latexmake}, there is a Makefile
to produce PDFs and graphics. The \airq{dependency} is the file
