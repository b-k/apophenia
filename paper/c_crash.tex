...
Try typing \binline{C tutorial} into your favorite
...
The two most popular are \ind{EMACS} and \ind{vi}. EMACS is better for people who
...
encouraged to start learning one or the other now. If neither suits your
fancy, there are literally hundreds of others to choose from. That said,
I will assume you have a good text editor (by your personal definition
of `good'), and will turn to the text you will be writing with it.
...
\cinline{float}\ttindex{float} is a floating-point number, aka a real number.\footnote{Why `floating point'? The
...
line, and could initialize \cinline{counter} to zero right when we declare it. The other
variables (such as \cinline{a_variable}) have unknown value right now. Assume nothing about
what is contained in a declared but uninitialized value.
...
\cinline{int a\_2d\_list[100][100];}\\
...
nor Perfect; just follow the template there and nobody gets hurt. Here's
another example:
...
topic we will return to later.  You'll need to declare the type of all of
...
halt whatever it's doing and start working on evaluating the function \cinline{do\_something}.
...
The \cinline{main} part of the program above didn't care what \cinline{do\_something} actually did, and had no
...
a separation of function code and function use we will make endless use of below.
...
\paragraph{The {\tt return} statement} \ttindex{return} There's no need to restrict ourselves to just one \cinline{return}
...
\paragraph{The {\tt main} function}\ttindex{main}
...
\cinline{ratio = a / b;}\\
will find the value of \cinline{a} divided by \cinline{b} and put the value in ratio. Notice that there's a
...
\cinline{a++; /*is equivalent to*/ a = a + 1;}\\
\cinline{a--; /*is equivalent to*/ a = a - 1;}
}
...
((a < 0) || (sqrt(a) < 3))
\end{lstlisting}
will always work: if \cinline{a} is less than zero, then the evaluation of the expression is done (it's true),
...
math. Since all comparisons evaluate to a zero or a one, both \cinline{( (a > b) || (c > d) )} and 
\cinline{(a > (b || c) > d)} make sense to C. You probably meant the first, but unless you have
the order-of-operations table memorized, you won't be sure which of the two C thinks you mean by\\ \cinline{(a > b || c > d)}.
...
Also, notice the semicolons, wich come after every line in the program that isn't flow control. The \cinline{if (a > 0)} statement doesn't do anything by itself, just direct flow, so there must be no semicolon following
it.
...
and a \cinline{*/} around the code, and the compiler will think it's a
...
a warning. If you're not using gcc, you may have to use the \cinline{/*} --- \cinline{*/} format for comments
...
programs. A copy is currently availaable at \url{http://www.bloodshed.net/dev/devcpp.html}.
...
includes the library {\tt gsl} (which we will use throughout this book); and it will output an executable
...
\cinline{#include <gsl/gsl_matrix.h>}\\
...
{\tt \#include $<$apophenia/headers.h$>$}\par
at the top of your file, you shouldn't need to include any of the other standard headers 
that one would normally include in a program for numerical analysis
({\tt stdio.h}, {\tt stdlib.h}, {\tt math.h}, {\tt malloc.h}, {\tt gsl\_anything.h}).
This means that you could ignore the headers at the top of all of the
code snippets in this chapter.\par
Of course, you will still need to include any headers you have written,
and if the compiler complains about an undeclared function, then its
header is not included in {\tt apophenia/\-head\-ers.h}.
Based upon all the notes about carefully-defined scope in this chapter,
such an undiscriminating header is bad style, but it is awfully
convenient, and will save you time hunting for standard headers.
}
...
Notice that \binline{file2.o} didn't get recompiled, since it is still
...
be able to compile quickly with a simple \binline{make} instead of the
...
program, but it's not fatal, so the program continues along using bad
...
error to a file: in the bash shell, which you're probably using: \binline{valgrind my\_program 2> problems}.
...
lives in a repository. When you want to work on the thing, you check out
a copy of the project, and when you're done making changes, you check
...
\cinline{REALLOC(var_array, new_length, int);}
...
as in optimization, not a zero. There's also an \cinline{-O1} and an \cinline{-O2}, but as long as you're optimizing, why not go all out?]
...
you step through the code, and when you ask to print the value of \cinline{a}, you'll get an error that there's no such variable defined.
