...
Sections \ref{fncontents} and  \ref{declaring} start small, covering the syntax of individual lines of code to assign, do arithmetic, and declare variables.
...
\cinline{b\_c}.\footnote{Referring to the number the pointer points to
is known as \vocab{dereferencing}. I will avoid using
the term, but you will see it frequently in error messages.}
...
\begin{lstlisting}
\cinline{int *k = malloc(sizeof(int));}
\end{lstlisting}
...
uses it, and returning that address. The input to malloc the
...
dereference a null pointer'. Finally, \cinline{k} could point to an area of
