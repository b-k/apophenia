...
\ifbook 
\index{header file!aggregation} The Apophenia library provides a convenience header that aggregates
almost every header you will likely be using. By placing
%[
...
data and get a segfault. This is a pain to trace using {\tt gdb}, so
...
using the efence library (adding {\tt -lefence} to the compilation
command).
...
{\tt gdb} alone. If anything breaks the rules, Valgrind will give you
the location, in the form of a backtrace very much like the backtraces
familiar from {\tt gdb}. It can even be set to start the debugger as soon
as it catches something wrong.\footnote{Use the {\tt db-attach} switch.
E.g., {\tt valgrind -v --db-attach=yes ./my\_program}.}
...
the {\tt break} will no longer be extraneous and will prevent a fall-through bug.
