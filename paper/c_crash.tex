...
The first element you will need is a \vocab{compiler}. If you are using
...
value in \ci{ratio}. Computer scientists often write this as \ci{ratio}
$\leftarrow$ \ci{a/b}, which nicely gets across an image of \ci{ratio}
taking on the value of \ci{a/b}. Notice that there is a semicolon at the end of
...
Second, \comment{you will occasionally use these in normal arithmetic or an assignment. This line:
\begin{lstlisting}
b = c == d;
\end{lstlisting}
...
The }the primary use of these conditionals is in flow control: causing
...
\end{lstlisting}
If \ci{a} is positive, then \ci{b} will be given the value of \ci{a}'s
square root; if \ci{a} is zero or negative, then \ci{b} is given the
value zero. As promised, the condition in line two requires parens around
it---a bare \ci{if a > 0} will produce \bi{syntax error before a}.

Curly braces, like those embracing lines three and six, indicate a block of
code that should be considered as a unit. The first block of code (line
three) is what gets run if \ci{(a>0)}, and the second block (line six)
is what gets run otherwise.

The \ci{else} clause is optional. If this snippet cut off at line three,
then nothing at all would happen when \ci{a<=0}.
...
do put a semicolon after an \ci{if} statement (e.g., \ci{if (a $>$ 0);},
then your \ci{if} statement will execute the null statement \ci{/*do
nothing*/;} if true. Your compiler will warn you of this.} and never
...
of an array, often named something like \ci{map} or \ci{apply};
in C, the best means of doing this is via a \ckeyind{for}\ci{for}
loop.\footnote{For those who want something slicker than a \ci{for}
loop, Creating \ci{map} and \ci{apply}-type functions is also
covered in Section \ref{apply}.} The following \ci{for} loop is exactly
equivalent to the \ci{while} loop above, but puts all the instructions
...
\end{lstlisting}
Naturally, you would want to run \ci{eval\_fn()} at least once, and the
\ci{do-while} loop guarantees that this will happen.

\exercise{Rewrite \bi{hello\_world.c} to print its greeting five times,
using a \ci{while} loop. Then redo this using a \ci{for} loop and a
\ci{do-while} loop.}


\paragraph{The birthday example} The \ind{birthday paradox} is a staple
...
\codefig{birthday}{Print the odds that other students share my birthday,
and that any two students in the room share a birthday.}
\lstset{numbers=none}

Listing \ref{birthday} shows code to find the likelihood that another
...
preface \ci{\#include}-ing a few external files, and a list of the
dramatis person\ae: variables named \ci{no\_match}, \ci{matches\_me},
and \ci{ct}.

Line 8 prints a header line labelling the columns of numbers the
\ci{for} loop will be producing. The syntax will be discussed in great
detail below, but for now note that \ci{\textbs{}t} prints a tab and
\ci{\textbs{}n} prints a newline. Line 9 begins the \ci{for} loop
itself. The counter \ci{ct} will start at two, and count up until it reaches 40.
...
material, \ci{no\_match} was initialized at 1, and on line 11, 
another element of Equation \ref{birthmath} gets
multiplied in to \ci{no\_match} 
at each step of the \ci{for} loop.

Line 12 prints the results. The first element in the \ci{printf} is a
...
printed: the counter \ci{ct}, \ci{matches\_me}, and \ci{1-(no\_match)}.

\exercise{Modify the \ci{for} loop to verify that the program prints the
right values for a class of one student.}
...
\comment{
Before you get intimidated further by \ci{printf}'s syntax, bear in mind
that if you have anything more than a handful of numbers, they will be
held in a data structure like a \ci{gsl\_vector} or an \ci{apop\_data}
set, which all have functions that will neatly print their contents.
This will spare you a great deal of complication, but you will still
use \ci{printf} to output the occasional scalar, and even to produce
database queries.
}
...
types of placeholders one could put in a \ci{printf} statement.
...
refer to any of a number of detailed references when you need these. To
give one example, the \ci{\%.3f} form prints a floating-point number
with three decimal places of precision.
There are a few more type specifiers that you will rarely use on page
\pageref{printftwo}.

Of course, we need to specify {\em which} integer, real, or string to insert, and the list of names
...
\exercise{Modify the \ci{printf} statement in \ci{birthday.c} to print
the odds as a percentage. That is, multiply the variables to print by 100,
and add percent signs to the format specifiers. If you would like to
keep the original intact for reference, make a copy of the line, then
...
Second, the example shows that every variable has a \vocab{type}. Lines two and
...
} \summarynoitems{(continued) \begin{itemize}
...
\index{declaration!of variables|(}\vocabmarker{declaration}
...
(\ci{int} were declared at once.  A variable that is declared but
...
line as it is declared. On line one, the \ci{counter} variable is declared
and initialized to zero at the same time.  However, both \ci{a\_variable}
and \ci{stuff} currently have unknown values.
...
\paragraph{Arrays} An \vocab{array} is a numbered list of items of the same type. To declare a list of a hundred
...
\subsection{\treesymbol Type casting}\label{casting} \index{casting|see{type casting}} \index{type casting|(} \vocabmarker{type cast}
...
switch in type: we just got an integer (\ci{3}) and are assigning it to
...
\subsection{Frames} \index{frames|(}
...
\vocab{frame} for that function. Into that frame are placed any variables
...
One way to think about this is in terms of a \vocab{stack} of frames.
The base the stack is always a function named \ci{main}.  For example,
in the program in Listing \ref{dosomething} the computer will ignore the
function \ci{do\_something}, instead starting its work on line eight,
where it finds the \ci{main} function head: \ci{int main(void)}. It
will create a \ci{main} frame and then start working, first reading the
declarations of \ci{output}, \ci{first}, \ci{second}, and \ci{third}
...
\subsection{Scope}	\label{scope} \index{scope|(}
...
A variable's \vocab{scope} is the set of functions that can see the
...
file which \ci{\#include}s that header file (see page 
...
\ind{C++}? The key difference is that C++ adds a second scope paradigm
on top of C's file- and function-based scope: \ind{object-oriented}
scope. In this system, functions are bound to objects, where an object is
effectively a \ci{struct} holding several variables and functions. Variables
that are \airq{private} to the object are in scope only for functions
bound to the object, while those that are \airq{public} are in scope
...
new keywords (\ci{public}, \ci{private}, \ci{protected}, \ci{friend},
\ci{virtual}), and on top of this, C++ has other features as well---yet
another type of scope called \airq{namespaces}, templates, and a
...
\paragraph{Call-by-value} \blindvocab{call-by-value}
...
throughout this book is the \vocab{GCC}, the GNU Compiler Collection, which will
...
linking the two later on. To summarize the process:

\begin{enumerate}
\item Gather together all the relevant declarations.
\item Now that all the symbols are defined, translate function A into the
machine code that will construct frame A and execute instructions.
Repeat with function B, creating machine code to construct frame B.
\item Link together the frames, so when frame A calls frame B, the system 
knows where to go.
\end{enumerate}
...
to simply type \bi{make} instead of the mess above. See Appendix A
...
\subsection{The preprocessing step} \label{headers}\index{preprocessor|(}\vocabmarker{preprocessor}
...
\vocab{header file} and include that file in only the code files that need it.
...
Step two of the three-step frame-construction process is \vocab{compilation},
which consists of taking each \ci{.c} file in turn and
writing a machine-readable \vocab{object file}, so \ci{file1.c} will
...
a frame to execute the \ci{gsl\_matrix\_add} function; the \vocab{linker} links
...
\vocab{include path}, and linker errors mean that you need to debug the \vocab{library
...
on creating a utility library from the exercise on page \pageref{headerize}.
...
execution is achieved using a \vocab{debugger}. You may want to get into the
...
files, you may need to specify a file name: \ci{break file2.c:35}. Or,
you can specify a function name, and the debugger will stop at the first line
after the function's header: \ci{break
gsl\_rng\_uniform}.\footnote{Provided debugging symbols are
...
function or going to a subfunction. Alternatively, \ci{next} or 
\ci{n} will step through the function (which may involve backtracking) but
...
remembering which is which: \ci{s} is the slowest means of stepping,
\ci{n} slightly faster, \ci{u} still faster, and \ci{c} the fastest. In
this order, they spell \ci{snuc}, which is almost an English word
...
include \airq{assertions} that check that things have not gone askew
...
Say you are writing out the roots of the quadratic equation, $x = {-b \pm \sqrt{b^2 -4ac}\over 2a}$, and erroneously code the first root as:
\begin{lstlisting}
firstroot = -b + sqrt(b*b - 4 *a *c)/2*a;
\end{lstlisting}
There are basically no intermediate steps: you put in \ci{a}, \ci{b},
and \ci{c}, and the system spits out a bad value for \ci{firstroot}. Now say that you
instead wrote:
\begin{lstlisting}[numbers=left, numberstyle=\scshape]
firstroot  = -b;
...
sequence for clues about the error. Say that \ci{a=0.5}. As you step through and check the value 
of \ci{firstroot}, you find that on line three it does not change.
From there, the error is obvious: the line should have been either
\ci{firstroot  = firstroot/(2*a)} or \ci{firstroot  /=
2*a}.  Such a
...
Returning to the world of programming, 
a \vocab{pointer} is the location of a piece of
data---the email address of the metaphor above. When you
declare \ci{int k}, then the computer is going to put \ci{k} somewhere
...
\paragraph{Call-by-address v \ind{call-by-value}} 
\blindvocab{call-by-address} \index{frames} 
...
Pointers  fix the problem of changing variables within a function. The
...
\lstset{numbers=left, numberstyle=\scshape}
\codefig{callbyref}{Call-by-address.}
\lstset{numbers=none}
...
\marginalia{18}{Declaration vs Use}{The number one point of
confusion with pointers is that stars mean different things in a
declaration and a usage. The declaration \ci{int *p, i} means that
\ci{p} will be a pointer to an integer and \ci{i} is an integer, but in
a non-declaration line like \ci{i = *p}, \ci{*p} refers to the intenger
value that \ci{p} points to. It looks like the symbol \ci{*p} has changed
meaning, and in some respects it has. Just be clear as to whether you
are declaring or using a pointer, and use the star accordingly.

For example, let us say
that we are declaring a new variable \ci{p2} that will be initialized to
have the same value as \ci{p}. Then the declaration would be \ci{int *p2
= p}, because \ci{p2} is being declared as a pointer, and \ci{p} is
...
Returning to the code in Listing \ref{callbyref}, in the \ci{main}
...
idea what location \ci{k} points to until we initialize it. The
...
\paragraph{The joy of segfaults} \index{segfault|see{segmentation fault}}
...
valuable. This is a \vocab{segmentation fault}, since you attempted to refer
...
\ci{calloc} behind your back. Then the program will give you an answer
...
\subsection{\ci{assert}} \cindex{assert} \vocabmarker{assertion}
