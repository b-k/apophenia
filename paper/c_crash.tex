...
idiosyncrasies and conveniences of C syntax.
...
First, we will cover the nouns of the C language, and subsequent
...
complex data types which are collections of many subelements. \cindex{struct}
For example:
...
Every complete program will have a function named \cind{main}, which
is where the program will begin.  When this program runs, the computer will
ignore the function \cinline{do\_something}, instead starting its work
on line eight, where it finds the \cinline{main} function head: \cinline{int main(void)}. 
It will then start working, first reading the
declarations of \cinline{output}, \cinline{first}, \cinline{second}, and \cinline{third}.
...
function \cinline{do\_something}. Execution jumps to the head of that
function, on line 2.  The value of \cinline{first = 7.3}
...
\cinline{output}, and then printing this output to the screen (using
the rather ornery line beginning with \cinline{printf}).
...
\paragraph{The \cinlinetwo{return} statement} \cindex{return} There
...
function whose type is \cind{void}, meaning that nothing at all will be
...
for $a$ mod $b$. \index{mod|see{\%}} \index{\%}
...
that the \cinline{sqrt()} function isn't a fundamental part of C---you
...
One way to think about this is in terms of a \ind{stack} of frames. At the
...
\paragraph{The preprocessing step} \label{headers}\index{preprocessor|(}
...
\ifbook \index{header file!aggregation} 
...
\fi
The preprocessor has a dozen things it can do, but you will basically only care about one of them: \cind{\#include}. 
...
of the declarations for \binline{some\_functions.c} in a {\sl \ind{header
file}}, to be named \binline{some\_functions.h}.  The motivation for
...
look in the standard library path. Use \binline{"my\_file.h"} for header
...
As noted above, modules and functions are most effective and  reusable when as few
...
a \cinline{\textbackslash{}0} character. Thus, a string variable needs
...
\paragraph{The executive summary} This file is reprinted from the \airq{Setup}
