...
everything here (using \ci{malloc} three times in a sentence, for
...
more details about any of the points I mention here, just type \bi{C
...
[The \ci{sqrt()} function isn't a fundamental part of C---you
...
Your basic options for variable types are \ci{int, float, double, long double} 
...
C has other basic types, which are not worth caring about, though there
are some detailed notes about how numbers are represented in Section
\ref{numbers}.

Some languages have a \ci{boolean type} to represent the values one or zero.
...
it is named \ci{is\_even}, and the name is prepended by a type,
\ci{int}.  On line 3 of the short version, the function returns
...
1.337 to \ci{output}, and then printing this output to the screen
(using the rather ornery line beginning with \ci{printf}).
Finally, the \ci{main} function 
...
file which \ci{\#include}s that header file (see below).\footnote{If
...
into \ci{b\_c}. It gives \ci{a\_c} the value of twice \ci{b\_c} and the returns that value. But \ci{a}
itself did not change at all, even though \ci{a\_c} did change. Meanwhile, \ci{int globe} is not a copy of
...
line like \ci{int x = 7;} instead of \ci{int x;}), or define
...
writing a machine-readable object file: \ci{file1.c} will
...
is made on line 12, we pass \ci{k}, which is a pointer, not
...
By the way, notice that \ci{int *k = 7} will fail---the initialization on the declaration line is for
...
\end{lstlisting}
which you can read as \airq{clear and allocate}: it will run \ci{malloc}
and return the appropriate address, and will also set everything in
that space to zero, running \ci{*k = 0} for you. Notice the syntax,
...
in scope, and it has to free the memory when the frame that \ci{k}
...
An array is a sequence of one type of item, such as a thousand \ci{int}s. Here is some sample code to declare an array and fill it with square numbers:
\begin{lstlisting}
...
Notice that referring to an element of \ci{squares} looks a lot
...
Strings such as \ci{"hello"} are implemented as arrays of individual characters, followed by an invisible
null character, written as \ci{$\backslash$0}. This means that you have to think in terms of arrays when dealing
...
There are also a few more examples on page \pageref{printftwo}.
...
\ci{info break}. You can delete breakpoint number three with
...
See the manual (\bi{man gprof}) for futher details about reading
the output.
...
element of the array \ci{a} can either be called \ci{a[6]} or
\ci{*(a + 6)}.  Some people find one-liners and pointer arithmetic
...
In the above example, getting rid of the \ci{goto} would require
copying the various memory-freeing
tasks at the end of the function into the center of the \ci{for} loop.
Linus Torvalds,\index{Torvalds, Linus} the author of the Linux kernel, recommends the \ci{goto} to eliminate redundancy as above; other authors choose to never use
\ci{goto}.
...
and your fellow humans. If you want optimization, \ci{gcc -O3} will
