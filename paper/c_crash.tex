...
\ifbook
	\startonecol \chapter{C} \label{c_crash} \endonecol
\else
	\long\def\comment#1{}
...
\airq{accessories} subfolder of the \airq{applications} folder) to do most of the
...
a \cinline{main()} function, such as \binline{callbyval}, you will be
able to compile the program by typing \binline{make} at the command
...
if (a > 0)
    ...
    { b = 0; }
\end{lstlisting}
If \cinline{a} is positive, then \cinline{b} will be given the value of \cinline{a}'s square root; if \cinline{a} is
...
You can exclude the \airq{else} part if you don't need it (which is
...
\cinline{for (i=0; i<limit; i++)} form, and will recognize it to mean \airq{step
through the array}.
...
like 
\cinline{a\_2d\_list[0][0]} to refer to the first element of the first row.
...
the \cinline{gsl\_matrix}, which provides many advantages over the raw
2-D array.
...
So, all evaluation in the \cinline{main} frame halts, 
the system generates a new frame, and execution jumps to 
line 2. The value of \cinline{first = 7.3}
will be plugged in to \cinline{a}, and similarly for \cinline{b = 3.3} and
\cinline{c = 2}. Then the computer will calculate the value of \cinline{d
...
Finally, the \cinline{main} function 
...
\binline{\$?}, so \binline{echo \$?} will print its value, should you
be curious about it.
...
When the preprocessor is processing the file \binline{main.c} and sees
the line
\begin{lstlisting}
#include "a_file.h"
\end{lstlisting}
it finds the file \binline{a\_file.h} and dumps the entirety of the file,
verbatim, into \binline{main.c} at that point in the file. You will never
see the expansion (unless you run \binline{gcc} with the \binline{-E}
...
data. After all, when you declare \cinline{int k}, then the computer is
going to put \cinline{k} somewhere in memory. Perhaps with a microscope,
...
\vspace \baselineskip
\begin{itemize}
\item To declare a pointer to an integer, use \cinline{int *k}.
\item Outside the declarations, to refer to the integer being pointed to, use \cinline{*k}.
...
the usage. Since \cinline{*k} is an integer, we declare \cinline{int *k},
just as we'd declare \cinline{int j} when \cinline{j} is an integer.
...
The spaces around our stars do not matter, so use whichever of \cinline{int
*k}, \cinline{int* k}, or \cinline{int * k} you like best. General
custom prefers the first form, because it minimizes the chance that you
will write \cinline{int * k, b} (allocate a pointer named \cinline{k}
and an \cinline{int} named \cinline{b}) when you meant \cinline{int *k, *b} (allocate two pointers). The star also
...
function, \cinline{k} is a pointer, the address of an integer, as
indicated by the star in its declaration on line 10. To print the integer
being pointed to, as on line 13, we use \cinline{*k}.
Similarly on line 4: \cinline{doubling} takes a pointer to an integer,
which will be named \cinline{k\_c} (so \cinline{*k\_c} is an integer),
and a plain integer, which will be named
\cinline{b\_c}.

Now for the call-by-reference trick.  When the call to \cinline{doubling}
is made on line 12, we pass \cinline{k}, which is a pointer, not
an integer. The computer builds itself a frame, using a copy of
\cinline{k}---that is, a copy of the address of an integer. Both
\cinline{k} (in the \cinline{main} frame) and \cinline{k\_c} (in the
\cinline{doubling} frame) now point to the same piece of data.  Line 5,
\cinline{*k\_c = b\_c * 2}, tells the computer to go to the address
\cinline{k\_c}, and put into that slot of memory the value \cinline{b\_c
* 2}. When the frame is destroyed (and \cinline{k\_c} goes with it),
...
k now holds: 4
\end{lstlisting}
because \cinline{*k}---the integer \cinline{k} points to---has changed as a
...
idea what address \cinline{k} points to until we initialize it. The
...
uses it, and returning that address. The input to \cinline{malloc} is the
...
%\eject
\index{\&}\marginaliafixed{13}{The ampersand}{Every variable has an address, whether you
...
By the way, notice that \cinline{int *k = 7} will fail---the initialization on the declaration line is for
...
that space to zero, running \cinline{*k = 0} for you. Notice the syntax,
...
\cinline{free(k)} \cindex{free} when you are done with the pointer
\cinline{k}. You don't actually need to free variables at the end of
\cinline{main}, because when you leave your program the operating system
...
When you initialize a non-pointer variable, say \cinline{int k}, then all of
the care and feeding of that variable is the compiler's responsibility:
it has to allocate memory for \cinline{k}, check whether the variable is
in scope, and it has to free the memory when the frame that \cinline{k}
...
specify array indices using an integer: \cinline{int i=1; data[i] +=7;}
...
indices: \cinline{size\_t i=1; data[i] +=7}. Also, the \cind{sizeof}
operator returns a number of type \cinline{size\_t}. In practice,
you are fine with using \cinline{int} to refer to memory locations
and never using \cinline{size\_t}.}

But what happens when you fail in your duties and forget to allocate
memory? One possibility is that \cinline{k} happens to point to a space
that has something that can be interpreted as an integer. When you refer
to \cinline{*k}, the computer will search the location \cinline{k}
points to, return whatever junk is found there, and will process that
junk like nothing is wrong. Alternatively, \cinline{k} could be the
...
dereference a null pointer}.\index{dereferencing}\index{null pointer} Finally, \cinline{k} could point to an area of
protected memory, such as the memory that is being used for the operating
system or your dissertation. In this case, referring to \cinline{*k} will
...
run \cinline{calloc} behind your
...
\cinline{an\_array = malloc (...)} changed the value of the copy
\cinline{an\_array} to a new location in memory.  Again, the copy is
...
If \cinline{hello} were a pointer to an integer, lines three and four
would be familiar errors. Line three is assigning data to an address,
like \cinline{int *p = 7}, and line four assigns one address to another,
...
\cind{apop\_strcpy}, that do the \cinline{realloc} for you; see the
...
strings that we will specify later. For example, \cinline{"person \%s
...
\end{lstlisting}
This function behaves just like \cinline{printf}, but with the name of
...
your code to set breakpoints. Some are stand-alone programs like \binline{ddd} and others are integrated into IDEs. Their operation is much like 
...
by typing \cinline{run}, and wait for it to break. When it does, you will
...
any debugging symbols, then that means that you forget the \binline{-g}
...
in the current function.  Finally, \cinline{c} will continue along until the next
...
\cinline{float mean=0} at first and loop beginning with \cinline{i=0}; I used this slightly odd initialization
...
What happens if the user calls the function with a \cinline{NULL}
pointer? It crashes. What happens if \cinline{length==0}? It crashes.
...
quick-and-dirty way to do this is to use the \cinline{is\_even} function
from earlier:
\begin{lstlisting}
if (is_even(array[i]))
    evens   += array[i];
...
\subsection{\cinline{assert}} \cindex{assert}
The \cinline{assert} macro makes a claim, and if the claim is false, the
program halts at that point. This can be used for both mathematical
assertions, and for housekeeping like checking for \cinline{NULL}
...
without assertions, the -\binline{DNDEBUG} flag to the compiler (just
add it to the command line) will compile the program with all the
\cinline{assert} statements skipped over.

\exercise{The method above for taking a mean runs risks of overflow
errors. Rewrite it so that it calculates an incremental mean as a
...
Be sure to fill the function
with the appropriate assertions about the inputs.
For a solution, see the GSL \cinline{gsl\_vector\_mean} function, or the
code in \cinline{apop\_db.c}.}
...
\cinline{find\_means(array4, -3)} will fail appropriately.

Some programmers actually write the test functions first.  This is a
method very much akin to what your high school writing teacher told you
about writing an outline before writing down the details. Write a comment
...
\cinline{main} as appropriate. Then, when you add another test, you will
re-run all your old tests at the same time. Peace of mind will ensue.

\exercise{Write the test function for the incremental mean program you'd
written above. Did your function pass on the first try? 

Some programmers
(\ind{Donald Knuth} is the most famous example) keep a bug log listing
errors they have encountered. If your function didn't pass its test the
...
Figure \ref{makefig} is a Makefile for a program named \cinline{run\_me}, which has two {\tt
...
make from within the program. \binline{gdb} lets you do this,
...
\cinline{-pg}. Then, execute your program, which will produce a file named
\binline{gmon.out} in the directory, with the timings that the profiler
will use. Unlike the debugger's \cinline{-g} option, the \cinline{-pg}
option may slow down the program significantly as it writes to
\binline{gmon.out}, so use \cinline{-g} always and \cinline{-pg} only
when necessary.

Finally, call \binline{gprof ./my\_executable} to produce a
human-readable table from \cinline{gmon.\-out}.\footnote{\binline{gprof}
outputs to \ind{STDOUT}; use the usual shell tricks to manipulate
the output, such as piping output through a pager, \binline{gprof
./my\_executable | less}, or dumping it to a text file, \binline{gprof
./my\_executable > outfile}, that you can view in your text editor.}
See the manual (\binline{man gprof}) for futher details about reading
the output.
...
\paragraph{Labels and \cinline{break}} A line of code can be named, by simply providing a
...
Linus Torvalds,\index{Torvalds, Linus} the author of the Linux kernel, recommends the \cinline{goto} to eliminate redundancy as above; other authors choose to never use
\cinline{goto}.
