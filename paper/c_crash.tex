...
\ci{for} loop will be producing, and line 9 begins the \ci{for} loop
itself. As above, line 9 tells us that the counter \ci{ct} will start at
two, and count up until it reaches 40.

As for the math itself, it is easier to calculate the complement---the odds that
nobody shares a birthday. The odds that one person does not share the first person's
birthday is ${364\over 365}$; the odds that two people both do not share the
first person's birthday is $\left(364\over 365\right)^2$, et
cetera.\footnote{We assume away leap years, and the fact that the odds
...
so the odds that the first three do not share a birthday is:
\begin{equation}
\left(1 - \left(364\over 365\right)\right)\left(1 - \left(363\over 365\right)\right).
\label{birthmath}
\end{equation}
...
Line 12 prints the results. The first element in the \ci{printf} will be
discussed in detail below, but the next elements indicate what is to be
printed: the counter, \ci{matches\_me*100}, and \ci{1-(no\_match*100)}.
As you can verify by compiling and running the program, it prints the
odds in a percentage format rather than as probabilities $\in [0,1]$.
...
The first step is preprocessing, which does nothing but take text 
...
There are a dozen things the \vocab{preprocessor} can do, but you will basically only care about one of them: \cind{\#include}. 
...
Returning to the world of programming, there are many reasons to assign to
a variable the location of data. All of the assignments to this point have
been direct assignments of data; this section looks managing the location
of data. It is easy to confuse your data and its location---and C's syntax
may or may not help with keeping them straight---but you will see that
having the location of data solves a number of problems. The key problems
that they solve in C are handling references and implementing arrays.
...
This is where pointers come in. A \vocab{pointer} is the location of a piece of
