...
and then write down a likelihood function---Or try bootstrapping, where the assumptions for the Central Limit Theorem apply by construction; see  Chapter \ref{boot}. 
If you are
...
from the distributions of Section \ref{distlist}. The distributions above
...
\paragraph{$\chi^2$ test}\index{chi squared distribution@$\chi^2$ distribution} $[\sum_n (x-\overline
...
\paragraph{$F$ test\index{F distribution}}  This is the same cancel-the-$\sigma$s trick as with the $t$, but here the form
...
applicable. Apophenia provides the \cinline{apop\_paired\_t\_test}
...
You can either just eyeball the kurtosis and $3 \sigma^4$, and decide based on the scale of the variance
...
a confidence interval around $3 \sigma^4$ and state with some percentage
of certainty that the kurtosis is or is not where it should be. [Hint: the
CLT applies to the kurtosis, so you know it has a Gaussian
distribution.]
