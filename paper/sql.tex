...
querying data. SQLite is a single-user system, which is exactly right
for a data anlysis project. If you hope to use another database system,
you will need to learn the (typically vendor-specific) commands for
...
have a plain old column named \sinline{row\_name}, or
another identifier such as \sinline{social\_security\_no} that serves
this purpose.  
...
\sinline{row\_name} variable, then one could select rows \sinline{where
row\_name == "Joe"}, but that is simply selecting rows with the
characteristic of having a \sinline{row\_name} variable whose value is
\sinline{"Joe"}. That is, column names are {\it bona fide} names; row
names are just data.}
...
columns'): it will find all of the rows in a table named \sinline{pop}
where \sinline{population} in that row is less than or equal to 50, and
return all the columns for those rows.

Generally, the \sinline{select} statement gives a list of columns that
...
\paragraph{Select}
The \sqlind{select} clause will specify the columns of the table that will
be output. The easiest list is \sinline{*}, which means `all the columns'.

Other options:\begin{itemize}
\item Explicitly list the columns:\\ \sinline{select country, pop}
\item Explicitly mention the table you are pulling from:\\
\sinline{select pop.population, gdp.country}\\
This is unnecessary here, but will become
essential when dealing with multiple tables below.
\item Rename the output columns:\\
\sinline{select pop.country as country, gdp as gdp\_in\_millions\_usd}\\ 
Notice that if you do not rename \sinline{pop.country as country}, then
you will need to use \sinline{pop\textbackslash.country} in future
...
writing):\\
\sinline{select country, gdp*0.53 as gdp\_in\_GBP}\\
The \sinline{as gdp\_in\_GDP} subclause is again more-or-less
...
You can alias the tables, for easier reference. The clause \sinline{from
data\_tab1 d1, data\_tab2 d2} gives short names to both tables, which
can be used for lines like \sinline{select d1.age, d2.height}.
...
in the \sinline{from} section there is no \airq{as}: \sinline{from long\_file\_name lfn}.
...
know and love: \sinline{where ((d1.age > 13) or (d2.height >= 175)) and
(d1.weight == 70)}.

The \sinline{where} clause refers to the root data, not the output,
meaning that you can readily refer to columns that you do not mention
in the \sinline{select} clause.
...
\item The columns are generally specified in the \sinline{select}
statement. You can pull all the columns from the data using \sinline{select *}, 
or you can specify individual columns like \sinline{select a, b, a*b as
product}.
\item The data source is in the \sinline{from} clause, which is
typically a list of tables.
\item The row specification, generally in the \sinline{where} clause,
...
conditions, like \sinline {where (a == b) and (b <= c)}.
}

\subsection{Joining}\index{join!database|(}
...
with data $\left[\begin{matrix}a\cr b\cr c\end{matrix}\right]$; then \sinline{select * from table1, table2} will
...
country name,
then you can use either \sinline{t1.country} or \sinline{t2.country}; since
...
number of rows in the \sinline{data} table.

\exercise{Verify that \si{select * from population, gdp} produces 43,264
rows.}

\index{pivot tables}\index{crosstabs}\label{crosstabs}
\marginaliafixed{16}{Crosstabs}{In the spreadsheet world, we often get tables
...
sum()} and \sinline{avg()}. These take an existing row as an argument.
...
Feel free to specify multiple \sinline{group by} clauses; this creates what
...
The function \sinline{sum()} works the same. 

You can use \sinline{count} with the \sind{distinct} keyword to find out how
...
\exercise{Find the total GNP per capita for each World Bank grouping.
...
\si{classes} table. Add up total GNP in the region, and divide by total
...
functions in an SQL call that you make from Apophenia: \sinline{stddev
...
SQL: \sinline{sqrt, pow, exp, log}. These functions do what you expect:
\sinline{select log(income), pow(in\-come, 3), sqrt(income) from data}
...
\paragraph{\cinlinetwo{Union}} For example, \sindex{Union}
\begin{lstlisting}
select id, age, zip
from data_set_1
union
...
\paragraph{\cinlinetwo{Union all}}\sindex{Union all} If a line is duplicated in both tables,
...
\paragraph{\cinlinetwo{Intersect}}\sindex{Intersect} As you can guess,
...
\paragraph{\cinlinetwo{Except}}\sindex{Except} This is the subtraction: it returns only
...
is via  a \sind{create} statement and then an \sind{insert} statement
for every single line. The \sinline{create} statement simply requires a
list of column names; the \sinline{insert} statements simply requires a
...
The \sind{begin}-\sind{commit} wrapper, by the way, means that everything
...
If you have hundreds or thousands of \sinline{insert}s, you are
...
\sinline{from} clause. When you do, you will need to specify a
\sinline{where} clause, and probably the \sinline{distinct} keyword, to
...
%\begin{itemize}
\item SQL includes a few simple aggregation commands: \sinline{avg()},
\sinline{sum()}, and \sinline{count()}. Apophenia provides a few more
nonstandard aggregators for queries called using its functions. 
\item When aggregating, you can add a \sinline{group by} clause to
...
\sinline{union}, \sinline{union all}, \sinline{intersect}, and \sinline{except}
\item You can create tables using the \sinline{create} and
\sinline{insert} commands, but you are probably better off just reading
...
\paragraph{Subqueries}\index{subqueries} 
...
You first need to get a \sinline{count(*)} grouped by category, and then get an
...
Some notes on the \sind{having} keyword should go here.
...
tables already have such a row, named \sinline{rowid}, that is normally
hidden from view. It is a simple integer, counting from one up to the
number of rows, and does not appear when you query \sinline{select *
from table}. But if you query \sinline{select rowid, * from table}
...
\sinline{select * from data where (random()/(-(1<<31)-1.0)+1)/2  < 0.14}
...
information. It consists of the following columns: \si{type}, \si{name},
...
(line 7), and a check for the size of the query that had just been run
...
the query and returns nothing, which is Not very useful for a \sinline{select}, but perfect for
...
number of the returned table into a \cinline{gsl\_vector} or a \cinline{float}.
...
data to the \airq{normal form}. For most data collection, there is a
...
\subsection{Dummy variables} \index{dummy variables|(}
The \sind{case} command is the if-then-else of SQL. Let us say that you
...
backward from our intuition.  Then we can put a \sinline{case} statement in with the other
column definitions to produce a column that is one when
\sinline{binaryq} is affirmative and zero otherwise:
\begin{lstlisting}
    select id, 
...
\sinline{case} statement for each value the variable could take, but
...
\sinline{case district when district\_no then 1 else 0} for each and
