...
In terms of implementation, Apophenia uses \ind{SQLite}, by D Richard
...
\binline{sqlite3}, but there are many other alternatives that are more
reminiscent of the table view in the standard stats package; ask your
search engine for \airq{sqlite browser} or \airq{sqlite GUI}. These
...
Apophenia is distributed with a few standalone
...
try \cind{apop\_text\_to\_db} on the command line to produce an SQLite
...
have a plain old column named \sinline{row\_name}; usually there is
...
\comment{
\summary{
\item Each column of a database represents a different named variable;
each row represents the values of those variables for a single unnamed observation.
}
}
...
know and love: \sinline{where ((d1.age > 13) or (d2.height >= 175)) and (d1.weight == 70)}.
...
in a text file and using \sind{apop\_text\_to\_db}. The form
