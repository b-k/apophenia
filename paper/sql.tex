...
This chapter will primarily consist of an overview of SQL, which you can
...
\marginaliafixed{14}{Crosstabs}{In the spreadsheet world, we often get tables
...
commit;
\end{lstlisting}
The \sind{begin}-\sind{commit} wrapper, by the way, means that everything will happen
...
use multiple databases, see the \cind{apop\_merge\_dbs} function (which
...
even back to the database). If you want a quick-and-dirty picture of a
...
that pointer, there is no way to free the memory afterward.} 
\begin{lstlisting}
...
backward.  Then we can put a \sinline{case} statement in with the other
...
\cinline{pvalue} is large, say $> 95\%$, then we can reject the null
