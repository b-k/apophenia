...
be importing data from a text file now.  The \cinline{apop\_text\_to\_db}
...
Generally, the \sinline{select} statment gives a list of columns which the output
...
Explicitly list the columns: \sinline{select age, height, weight}

Explicitly mention the table you are pulling from: \sinline{select data.age, data.height, data.weight}. This is unnecessary here, but will become
essential when dealing with multiple tables below.

Rename the output columns: \sinline{select data.age as age, height as height\_in\_cm}. Notice that if you do not rename \sinline{data.age as age}, then you will need to use \sinline{data\\.age} in future
queries, which is a
bit annoying.
...
\paragraph{Pruning rows with \sinlinetwo{distinct}} The \ttind{distinct}
...
If \sinline{data1} has 15 lines, and \sinline{data2} has 8 lines, then \sinline{select *
...
(\sinline{create index econ\_index on econ\_data(zip, age)}), but as a
...
\end{lstlisting}
%\end{verbatim}
This produces a table with one column and one row, which lists the total number of rows in the \sinline{data} table.
...
interested in the \ttind{apop\_db\_to\_crosstab} function, {\em qv}.
...
expect: \sinline{select log(income), pow(income, 3), sqrt(income) from data} will give
...
\paragraph{\cinlinetwo{Intersect}}\ttindex{Intersect} As you can guess, \sinline{intersect} returns
...
If you have hundreds or thousands of \sinline{insert}s, you're almost certainly better off putting the data
...
the query and returns nothing. [Not very useful for a \sinline{select}; but see
...
\paragraph{apop\_query\_to\_data} \ttindex{apop\_query\_to\_data}
...
\end{lstlisting}
%\end{verbatim}
will query \sinline{data} for all its information, allocate \sinline{tall\_people} to be an
...
number into \cinline{gsl\_vector} or a \cinline{float}.

\paragraph{Data to db} For the other direction, there are the plain old
print functions: \ttind{apop\_data\_print} and
\ttind{apop\_matrix\_print}. If the global variable
...
an erratic state. If so, try the SQLite command \ttind{vacuum}.}
...
The \ttind{apop\_table\_exists} command checks for whether a table
...
Here's a neat trick: using SQL's \sinline{case} a few dozen times, we can turn a
...
\cinline{apop\_produce\_dummies} on the matrix side.
\ttindex{apop\_produce\_dummies}
