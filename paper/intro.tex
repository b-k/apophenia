...
\comment{
\def\olsfrm{(\Xv'\Xv)^{-1}\Xv'\yv}
\def\yysq{(\yv_{\rm out} - \yv)^2}
\def\yout{\yv_{\rm out}}
\begin{figure}[htb]
%\hskip -1.4cm
\begin{center}
\psfrag{Prm}[cc][cc]{$\betav$}
\psfrag{Dta1}[cc][cc]{$\Xv$}
\psfrag{Dta2}[cc][cc]{$\yv$}
\psfrag{xpxi}[cc][cc]{$(\Xv'\Xv)^{-1}\Xv'\yv$}
\psfrag{out}[cc][cc]{$\yv_{\rm out}$}
\psfrag{diffsq}[cc][cc]{$(\yv - \yv_{\rm out})^2$}
\includegraphics[width=\textwidth*\real{1.05}]{/home/klemens/ols}
%\input{ols.tex}
%\scalebox{0.95}{\includegraphics{ols.ps}}
\end{center}
\caption{OLS.}
\label{olsflow}
\end{figure}
...
function in the mathematical sense, $f:\Xv, \betav \to \yv_{\rm out}$. It may
be as simple as $\yv_{\rm out} = \Xv\betav$ or an elaborate flowchart where
...
\comment{
But if OLS is not appropriate, we have the technology to go much further.
 Before computers, we could only
...
I wrote this book to help you select appropriate models, and then make
...
The book also makes use of the Apophenia library, a set of
...
All of the tools and libraries
...
\codefig{regress}{A translation of Figure \ref{modelflowback} into C.}
\lstset{numbers=none}

With these tools in hand, the process of translating a statistical model
to computer code is relatively painless.
For example, Listing \ref{regress} shows the C translation
of Figure \ref{modelflowback} for the simple case of the OLS model. Line
three indicates that we are using these libraries listed above, and then
the translation flows easily: line
six reads data from a text file, line seven produces a set of parameter
estimates using the \ci{apop\_OLS} model, and line eight displays the
parameters and some other data on the screen. [The other lines will be
...
in equation form and in the form of executable code. As
...
I assume that you are computer-literate. On top of the basics of 
file- and directory-handling (either via a GUI or \bi{ls},
\bi{cd}, \bi{mkdir}, \bi{rm}, \&c),  you have some experience
...
are working in C, so we can have the best of all worlds.} Because there
...
Apophenia implements SQL via the \ind{SQLite} library, so that is the
...
your plots. Once you have the basics down, animation and real-time
...
Graphviz, a program to automatically produce graphs (i.e., networks of nodes
connected by edges).
...
\paragraph{Some notation}  \label{notation}\hfill
...
like this paragraph. There is much to be said for the strategy of
flipping ahead to the summary at the end of the section before reading
the section itself.
...
language designed to help manage large or complex data sets.
\end{itemize}
}
...
likelihood. MLEs have the pleasing property of meeting the Cram\'er-Rao
...
producing tables of just the right form from a data set. C and SQL are
