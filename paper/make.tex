...
You are probably using \bi{bash} or another variant of the Bourne
shell.\footnote{\bi{bash}= Bourne-again shell.} In these systems,
set environment variables using the shell's \bi{export} command:
\begin{lstlisting}[language=sh]
export USER=Steven
\end{lstlisting}
...
shows that \cind{getenv} will return a string holding the value of the
environment variable, and \cind{setenv} will set the given environemnt
...
via both environment variables and instructions on the command line.
}
