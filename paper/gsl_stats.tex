...
\def\vv{{\bf v}\xspace} \def\xv{{\bf x}\xspace} \def\yv{{\bf y}\xspace} \def\pv{{\bf p}\xspace}
...
%glossary + index
\def\airq#1{{\sl #1}}
\def\vocabmarker#1{\label{#1firstuse}}
\def\blindvocab#1{\index{#1}\vocabmarker{#1}}
\def\vocab#1{\airq{#1}\blindvocab{#1}}


%code listing:
\lstset{columns=fullflexible, basicstyle=\small, emph={apop_data,apop_model,apop_inventory,apop_estimate,apop_estimation_params,gsl_vector,gsl_matrix,gsl_rng,FILE},emphstyle=\bf\small}
...
\comment{
\long\def\codefigure#1#2#3{
	\begin{figure}[htb]
    \indwcsub{sample code}{#1#2}
	\hrule\vskip4pt
\lstset{frame=,showstringspaces=false, basicstyle=\small, emphstyle=\bf\small,%
language=C, breaklines=true}
\lstinputlisting{sources/#1#2}
	\hrule

\caption{#3 [Online source: \bi{#1#2}]}\label{#1}
    \setlistdefaults
	\end{figure}
}
}
...
\begin{figure}[tb]
    \indwcsub{sample code}{#1#2}
\lstset{float=htb,caption={#3 Online source: \bi{#1#2}.},label={#1},%
captionpos=b,xleftmargin=\mwidth,%
...
\long\def\codefig#1#2{\codefigure{#1}{.c}{#2}}

\long\def\birdfig#1#2{
	%\begin{figure}[htb]

    \indwcsub{sample code}{#1}
\lstset{showstringspaces=false, basicstyle=\small, emphstyle=\bf\small, language=C,%
breaklines=true,%
caption={#2 Online source: \bi{birds/#1}},captionpos=b,label=#1}
\lstinputlisting{sources/birds/#1}
	\vskip 0.5cm
    \setlistdefaults
	%\caption{#2 [Online source: \bi{#1}]}\label{#1}
	%\end{figure}
}
