...
\comment{(I should check this) }the BLUE of the true mean $\mu$. 
...
\item Write a query that pulls the number of males per 100 females and
the population density from the Census data (\bi{data-census.db}). The query will:
    \begin{itemize}
    \item join together the \si{geography} and \si{demos} tables by county name or number, and
    \item exclude states and the national total, and
    \item return a two-column table.
    \end{itemize}
\item Write a function \ci{void summarize\_paired\_data(char *q)} that takes in a query that produces a two-column
table, and outputs some of the above summary information about both
columns, including the correlation coefficient.
\item Write a \bi{main()} that sends your query to the above function;
run the program.
\end{itemize} (continued)} \addtocounter{ex}{-1}\exercise{(continued) \begin{itemize}
\item Write another query that pulls the ratio of 
(median income for full-time workers, female)/(median income for full-time workers, male) and
the population density. 
\item Add a line to \bi{main()} to send that query to your summarizing function as well.
\end{itemize}
...
percentiles to the screen. For example, if \ci{divisions==4}, print quartiles, if
...
\index{CDF|see{cumulative density function}}
...
data with the given distribution, a \vocab{probability density function} (PDF),
and a \vocab{cumulative density function} (CDF). The full details of RNG use
...
\eqnbox{ \label{normal}
P(x|\mu,\sigma)&=&{1\over \sigma\sqrt{2\pi}} \exp\left({-{1\over 2}\left[{(x-\mu)\over\sigma}\right]^2}\right)\\ 
...
belief that any value within $[\alpha, \beta]$ is equally possible.
...
others regarding \vocab{Zipf's law}, that cities,
...
\exercise{Above, you found a relationship between males per female (the
dependent variable) and
both density and male-female wage ratios (the independent
variables). Produce an appropriate data set and send it to
\ci{apop\_OLS.estimate} to check the coefficients when both independent
