\startonecol \chapter{Describing data} \label{projections} \endonecol
...
\cinline{apop\_vec\-tor\_var},
\cinline{apop\_vec\-tor\_skew}, and
\cinline{apop\_vec\-tor\_kurt\-o\-sis} all return the sample central
...
\end{lstlisting}
The last item in the code, \cind{apop\_data\_show} produces a table of
...
modeling. If you really think that a variable is the outcome of $n$
...
\vocab{excess variance}, which indiates that there are interactions that
falsify that the observations were iid Binomial events.
...
singular-value decomposition of a data set}
...
do. However, as
noted earlier, there is no true concept of \ind{causality} in statistics.
...
\cind{apop\_matrix\_norm\-al\-ize}.}
\end{itemize}
