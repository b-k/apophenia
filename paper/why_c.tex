...
R and Apophenia use identical code (in C) for doing the \ind{Fisher
exact test}, so it makes a good basis for comparison.  On my laptop,
...
slow.\footnote{By the way, I found that it takes a mere three
...
able to verify and modify your work five years from now?
Since C was written in 1972, dozens (maybe hundreds) of stats packages
have come and gone. Those who try to follow the trends 
have learned a half-dozen languages and have on their hard drives
dozens of scripts that they can't run anymore. Chapter \ref{c_crash}
shows you how to build a library of functions so you can put together next
month's analysis in half the time, but to profit from such efficiencies,
you need a stable language that will be around for the long haul. For
example, correctly written C programs written in the 1970s will compile
and run on new PCs.  
\index{statistics packages!rants regarding|)}
