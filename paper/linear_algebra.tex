...
\cinline{gsl\_matrix} with labels. On page \pageref{markovtwo}, the
progran is simplified using \cind{apop\_data}-specific functions.
...
That said, here are some notes on the GSL's structures for holding data,
which will be useful to you for understanding why the data structures
...
\paragraph{\ci{gsl\_matrix} structure}} Here is the declaration of the
...
\ci{sizeof(double)*2} steps from the base element.  Stepping along the row
means simply stepping along by \ci{sizeof(double)} units, and stepping by
column means stepping by \ci{sizeof(double)* size2} steps from the current
element. 
...
\ci{size2}.\footnote{The abbreviation \ci{tda} somehow stands for {\em trailing dimension}. It is also known as the \vocab{stride}.} Jumping down a column thus requires a jump of
...
and others could be \ci{sizeof(double)}. Since space is always allocated
...
\item There is no simple way to take non-contiguous subsets of matrices
or vectors. You will either need to copy the data manually (i.e., using
