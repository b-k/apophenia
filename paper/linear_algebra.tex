...
That done, the \cinline{for} loop put some values in the first column of the matrix. 
...
\cinline{m-$>$size1}. You'll recognize this as accessing a struct, which is exactly
what we're doing: the declaration \cinline{gsl\_matrix *m} means that \cinline{m} is a
...
type such as \cinline{apop\_data}, \cinline{apop\_estimate}, \&c.
...
could write a file \cinline{my\_convenience\_fns.c}, which would include:
...
You'll also want a header file, \cinline{my\_convenience\_fns.h}:
%\begin{verbatim}
\begin{lstlisting}
double mget(gsl_matrix *m, int row, int col);
double vget(gsl_vector *v, int row);
...
After throwing an \cinline{\#include "my\_convenience\_fns.h"} up at the top of your
program, you'll be able to use your abbreviated syntax such as \cinline{mget(v,3)}.
...
i = 1/0} and you'll get \cinline{Arithmetic exception (core dumped)}.
...
matrix. Here's how to get the fifth row of \cinline{a\_matrix} into a vector view:
...
For the fifth row, use \ttind{gsl\_matrix\_row}\cinline{(a\_matrix, 4)}. 

The vector view is a data structure which includes an element of type \cinline{gsl\_vector}
named vector; this is the only element you will be interested in. The
expression \cinline{\&(v.vector)} is of type \cinline{gsl\_vector *}, and therefore can
be used as you would any other pointer to a \cinline{gsl\_vector}. For example,
try \ttind{apop\_mean}\cinline{(\&(v.vector))};.
...
or \cinline{CblasNoTrans}. For example, here's $X'Y$:
...
code; \ttind{apop\_det\_and\_inv} does indeed invert and calculate the
...
return $\beta$. This is how \cinline{apop\_OLS} will find $\beta$ if $\beta$
is all you ask for.
