...
first argument.
\cinline{CBLAS_TRANSPOSE_t} is a type defined by the GSL that can take
...
Here is a complete program to read in this matrix and
then output this count of two-step transitions. The steps should be clear: read the data, allocate a space for the output, calculate the dot product, and print the result.
...
To that end, here are the data types discussed in this book, and
suggestions for converting from one to the other. It is not an exciting
...
From/to pairs marked with a $\cdot$ are
...
only transitions we can not do in two steps are \cinline{apop\_data}
$\Rightarrow$ \cinline{double[ ]} and plain text $\Rightarrow$
\cinline{double[ ]}.
...
\paragraph{Method C: Copying} I read the process of converting from a
given type to itself as the process of copying. Most of the types above
have a method of copying; some even have a few options allowing for
different tastes.
...
//\cinline{gsl\_vector} $\Rightarrow$ text file
    ...
    apop_vector_print(original, "text_file_copy");
//\cinline{gsl\_matrix} $\Rightarrow$ text file
    apop_opts.output_type = 't'
    apop_matrix_print(original, "text_file_copy");
//\cinline{apop\_data} $\Rightarrow$ text file
    apop_opts.output_type = 't'
    apop_data_print(original, "text_file_copy");
//\cinline{gsl\_vector} $\Rightarrow$ db table
    apop_opts.output_type = 'd'
    apop_vector_print(original, "db_copy");
//\cinline{gsl\_matrix} $\Rightarrow$ db table
    apop_opts.output_type = 'd'
    apop_matrix_print(original, "db_copy");
//\cinline{apop\_data} $\Rightarrow$ db table
    apop_opts.output_type = 'd'
    apop_data_print(original, "db_copy");
\end{lstlisting}
