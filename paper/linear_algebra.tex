...
and \ci{gsl\_matrix\_...} sections of this index or the GSL's online
...
\paragraph{Precision}\index{precision, numerical} 
...
add ten to $n$, so operations like doubling or halving a number will not
...
right from the start. \label{printftwo} A few more \cind{printf} format
specifiers are also given: \ci{f} means \ci{float}, \ci{lf} means
\ci{double} (which used to be known as a long float), the \ci{-} means
that the number should be left-justified, and the number \ci{\%65.63}
...
represented by a \ci{double}. Notice that for $i>1,000$, a
modest number of data points, the system throws in the towel and just
calls $2^i = \infty$ and $2^{-i} = 0$. For those who would like
...
\item Reporting results based on the fifth significant digit is spurious.
}
