...
\paragraph{The \vocab{Central Limit Theorem}} The CLT is the first piece of magic
...
\subsection{Student's $t$ distribution\index{t distribution}} \label{tstat}
Let $\uv$ be
...
cancels out from the numerator and denominator.\footnote{\airq{Student}
is actually Mr. William Sealy Gosset, who published the $t$ test in 1908
based on his work as an employee of the Guinness Brewery. \index{Student}
\index{William S Gosset}}

\begin{itemize}
\item The $t_1$ distribution (i.e., $n=2$) is called a \ind{Cauchy distribution}.
\end{itemize}
...
Or, say we want to test that $\beta_2 = 2\beta_3$. Then let $\Qv=\vector{0 \cr 1 \cr -2}$ and $\cv = 0$. In ANOVA terminology, a hypothesis about a linear combination of coefficients is known as a \vocab{contrast}.
...
database discussion, it is a \airq{crosstab}, which differs slightly
