...
Figure \ref{rng_alloc} shows a function to initialize a \cind{gsl\_rng}.
...
time. There are a number of functions that will turn this into a time
that we humans would find useful; see any reference on C's standard
...
code online), and run it continuously from the shell. In \bi{bash}
...
sum $$\frac{f(-500.00)\cdot p(-500.00)}{100000} + \frac{f(-500.01)\cdot
p(-500.01)}{100000} + \cdots + \frac{f(500.00)\cdot p(500.00)}{100000}.$$ This can be effective, but there are
...
kurtosis. This has no effect for $n >$ 1 million, but since it takes
...
just as the variance is undefined for a Cauchy distribution (i.e., a $t$
