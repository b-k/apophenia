...
would initialize a \ttind{gsl\_rng}:
\begin{lstlisting}
...
\paragraph{The Beta distribution}\index{Beta distribution}
...
\cind{apop\_\-ran\-dom\_\-beta}. You can give it a mean, a variance, and
...
$\in[0,1]$. If you send \cind{apop\_\-ran\-dom\_\-beta} values of $\mu$
...
Then, the \ttind{gsl\_histogram\_pdf\_sample} function will
...
a \ttind{gsl\_histogram\_pdf} structure from which draws can be
...
calc\_statistic(gsl\_vector *da\-ta)} that finds some statistics. The
...
\end{lstlisting}
In fact, this is redundant, because the \cinline{apop\_\-pois\-son}
model already uses the \cind{apop\_\-boot\-strap} function internally,
so the usual \cinline{apop\_\-pois\-son.est\-i\-mate(data, NULL, NULL)}
