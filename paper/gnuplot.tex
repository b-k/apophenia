...
\subsection{\gi{splot}}\gkeyind{splot} The \ci{plot} command prints flat, 2-D plots.
...
easy to do using a \ind{pipe}, so named because of UNIX's running
...
Once everything is neatly aggregated, the function opens a file and dumps the data to a Gnuplottable file. There is an example of this function in use in Listing \ref{drawbeta}, page \pageref{drawbeta}.
\begin{lstlisting}
...
\section{Animation}\label{animation}\index{animation} Perhaps three dimensions is not
...
For more examples of animation production, see
the sample code in Listings \ref{normalgrowth} and \ref{bimodality}.
...
typically mean the word \vocab{graph} to be a plot of data like every
...
The online code supplement includes a few more examples of Graphviz at
work---notably, the code used to create Figures \ref{listfig},
\ref{btreefig}, and \ref{viewfig} in this book.
