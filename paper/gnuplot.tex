...
\airq{fork} the code base). The project is entirely unrelated to the GNU,
...
'plotme'}. This leaves you at the Gnuplot prompt to expermient with settings.
...
void animate_data_sets(apop_data *d, int ct, char *outfile){
  ...
  FILE  *f = fopen(outfile, "w");
    ...
    for(i=0; i< ct; i++){
        fopen(outfile, "a");//append, don't overwrite!
        if (i && i< ct-1)
            fprintf("e\n\
                pause p;\n");
        fprintf("splot '-' with pm3d\n");
        fclose(f);
...
Since Gnuplot lets us select colums via the \ci{using 1:5} clause, there
...
generates documentation via specially-formattec comments in the source
