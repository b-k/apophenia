...
First, write sources/code.c, then (in the sources dir) run
c++2latex -a code.c
to produce code.tex. The first argument to codefig will be
...
with no length specified. \index{array!declaration} \index{declaration!of arrays}
...
(with {\tt ls}), then you'll see that you now have your original {\tt .c} files, but also new files with a
{\tt .o} ending and the {\tt run\_me} executable program. You can type {\tt ./run\_me} and see your
program work. 
...
\section{The debugger} \index{debugging|(} \index{gdb@{\tt gdb}|see{debugging}}
...
\index{segementation fault} \label{valgrind}
...
\ttind{GSL\_MAX} is similarly defined.
