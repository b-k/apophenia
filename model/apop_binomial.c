...
static double binomial_log_likelihood(const apop_data*, apop_data*, apop_model*);
...
static apop_model * binomial_estimate(apop_data * data,  apop_model *parameters){
  ...
  double		mean, var;
    ...
    est->llikelihood	= binomial_log_likelihood(est->parameters, data, parameters);
    apop_numerical_covariance_matrix(apop_binomial, est, data);
	return est;
}

static double binomial_log_likelihood(const apop_data *in, apop_data *d, apop_model *params){
int		    i,j;
double	    x, 
            loglike = 0,
...
static double binomial_p(const apop_data *binomial, apop_data *d, apop_model *p){
    return exp(binomial_log_likelihood(binomial, d, p));
}

static double binomial_constraint(const apop_data *binomial, apop_data *returned_binomial, apop_model *v){
