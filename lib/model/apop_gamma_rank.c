...
static apop_estimate * gamma_rank_estimate(apop_data * data, apop_inventory *uses, void *parameters){
apop_inventory  actual_uses    = apop_inventory_filter(uses, apop_gamma_rank.inventory_filter);
    return apop_maximum_likelihood(data->data, &actual_uses, apop_gamma_rank, *(apop_estimation_params *)parameters);
}
