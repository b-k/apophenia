...
\li \c apop_model_free will free the parameter set. If the data is simply a view of data that is really owned by somebody else, then that means I can't keep it in the <tt>model->parameters</tt> slot. So: if the data is generated by the user and held by \ref apop_pmf, then it is stored in the \c more pointer; if it is generated by \ref apop_crosstab_to_pmf then it is held in the \c parameters section. This is obviously a hack, and may be refined in the future.
